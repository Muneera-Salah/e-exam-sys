                            ...
                            <div class="btn-toolbar float-md-right" role="toolbar">
                                <div class="btn-group" role="group">
                                    ...
                                    <a href="{{ route('formbuilder::forms.index') }}"
                                        class="btn btn-primary float-md-right btn-sm back-icon">
                                        <i class="fa fa-arrow-left"></i>
                                    </a>
                                    <a href="{{ route('formbuilder::forms.submissions.index', $form) }}"
                                        class="btn btn-primary float-md-right btn-sm data-icon">
                                        <i class="fa fa-th-list"></i> Submissions
                                    </a>
                                    @if (Auth::user()->isAdmin())
                                        <a href="{{ route('formbuilder::forms.edit', $form) }}"
                                            class="btn btn-primary float-md-right btn-sm edit-icon">
                ...
                <div class="card rounded-0">
                    <div class="card-header">
                        <h5 class="card-title">
                            Details
                            @if (Auth::user()->isAdmin())
                                <button class="btn btn-primary btn-sm clipboard float-right link-icon"
                                    data-clipboard-text="{{ route('formbuilder::form.render', $form->identifier) }}"
                                    data-message="Copied" data-original="Copy Form URL" title="Copy form URL to clipboard">
